public class SnacksCompartment implements IBag<Item> {
    Item[] snacks = new Item[30];
    int limitCompartment = 2000;
    int totalWeight = 0;
    int indexCount = 0;
    ...
    public Item removeByIndex(int index) {
        if (snacks[index - 1] != null) {
            Item removedItem = snacks[index - 1];
            snacks[index - 1] = null;
            totalWeight -= removedItem.amount;

            for (int a = index - 1; (a - 1) != snacks.length - 2; a++) {
                Item temp = snacks[a];
                snacks[a] = snacks[a + 1];
                snacks[a + 1] = temp;
            }
