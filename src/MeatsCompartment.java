public class MeatsCompartment implements IBag<Item> {
    Item[] meats = new Item[30];
    int limitCompartment = 5000;
    int totalWeight = 0;
    int indexCount = 0;
    ...
    public Item removeByIndex(int index) {
        if (meats[index - 1] != null) {
            Item removedItem = meats[index - 1];
            meats[index - 1] = null;
            totalWeight -= removedItem.amount;

            // Swap
            for (int a = index - 1; (a - 1) != meats.length - 2; a++) {
                Item temp = meats[a];
                meats[a] = meats[a + 1];
                meats[a + 1] = temp;
            }
